  ...
  return {
    ...
    html: base('Подтверждение данных для заявления', `
      ...
      ${table(
        ...
        detail('Предварительный вариант — требует проверки', escapeHtml(input.proposedRegistryName))
      )}
      ${para('Для правильного заполнения заявления необходимо уточнить личную фамилию матери и проверить, менялась ли она при браке. Фамилия в паспорте или TIE может быть фамилией по браку. Удвоение фамилии не является свободным выбором для отказа от документов.')}
      ${para('Пока не заказывайте новые свидетельства и переводы. Сначала проверим имеющиеся документы: полное свидетельство о рождении ребёнка и, при наличии, свидетельство о браке с фамилией матери до брака. Если их недостаточно, уточним допустимую альтернативу.')}
      ...
      ${para('<strong>Пожалуйста, уточните:</strong> какая фамилия была у матери при рождении, менялась ли она при браке и в каком из уже имеющихся документов это указано. Пришлите такой документ через безопасную область клиента, если он есть.')}
      ${para('После ответа проверим документальное основание и согласуем корректные данные перед отправкой бланка на подпись.')}
    `)
  };
}
