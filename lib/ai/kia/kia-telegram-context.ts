...
export async function findCaseConversation(admin: Admin, profileId: string, caseId: string, companyId: string | null) {
  const { data: owned, error: ownerError } = await admin.from('cases').select('client_id,company_id')
    .eq('id', caseId).eq('client_id', profileId).maybeSingle();
  if (ownerError) throw ownerError;
  if (!owned || (owned.company_id ?? null) !== companyId) return null;
  const { data, error } = await admin.from('kia_conversations').select('id,company_id')
...
export async function loadTelegramCaseContext(input: { admin: Admin; profileId: string; tenantId: string | null;
  ...
  if (token) {
    ...
    return { caseId: context.case_id as string, companyId: (context.company_id ?? null) as string | null,
      ...
      stored: await findCaseConversation(input.admin, input.profileId, context.case_id, context.company_id ?? null), opened: true };
  }
  const { data, error } = await input.admin.from('kia_conversations').select('id,case_id,company_id,service_slug')
  ...
  const stored = await findCaseConversation(input.admin, input.profileId, data.case_id, data.company_id ?? null);
