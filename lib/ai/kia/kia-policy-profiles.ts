...
const POLICY_PROFILES: Record<KiaPolicyProfileName, KiaPolicyProfileDefinition> = {
  ...
  professional_operator: {
    ...
    // M4.1 intentionally keeps the active ceiling at R1. R2+ is staged only.
    maxRiskTier: 'R1',
  ...
  internal_admin: {
    ...
    // Legal/economic actions remain unavailable until a later explicit policy milestone.
    maxRiskTier: 'R1',
...
export function resolveKiaPolicyProfile(
  ...
  if (missingPlanCapabilities.length > 0) {
    return {
      ...
      missingScopes,
      missingFeatureFlags,
      reason: 'missing_plan_capability',
    };
  }
