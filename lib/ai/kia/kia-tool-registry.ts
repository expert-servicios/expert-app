...
/**
 * Authoritative tool selection. Any tool without registry policy is excluded
 * (fail-closed), even if a model/client asks for it by name.
 */
...
export function isKiaToolAuthorized(name: string, context: KiaToolAuthorizationContext): boolean {
  return resolveKiaToolDefinitions({ ...context, requestedNames: [name] })
