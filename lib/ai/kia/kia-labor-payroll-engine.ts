...
export type LaborFindingCategory =
  | 'employee'
  | 'contract'
  | 'schedule'
  | 'salary'
...
export function analyzeLaborPayrollSnapshot(snapshot: LaborPayrollSnapshot): LaborPayrollDiagnosticResult {
  ...
  if (!contract) {
    ...
    if (contract.scheduleHours == null) {
      findings.push({ code: 'schedule_hours_missing', severity: 'warning', category: 'schedule', summary: 'La fuente no devuelve horas de jornada en el contrato activo.' });
    }
    if (!contract.scheduleMode) {
      findings.push({ code: 'schedule_mode_missing', severity: 'warning', category: 'schedule', summary: 'La fuente no devuelve modalidad de jornada en el contrato activo.' });
    }
    if (!contract.workingDays?.length) {
      findings.push({ code: 'working_days_missing', severity: 'warning', category: 'schedule', summary: 'La fuente no devuelve días de trabajo en el contrato activo.' });
    }
  ...
  if (snapshot.payslipsReadError) {
    findings.push({
      ...
      severity: 'review',
      category: 'payslip',
      summary: 'No se pudieron leer las nóminas calculadas para el periodo solicitado.',
      evidence: { readError: snapshot.payslipsReadError },
    });
    ...
    findings.push({
      ...
      severity: 'warning',
      category: 'payslip',
      summary: 'No hay nóminas calculadas para el empleado y periodo consultados.',
    });
  }

  const payslips = snapshot.payslips.map((payslip) => {
    const contributionBasesAvailable = Object.keys(payslip.contributionBases ?? {}).length > 0;
    const irpf = detectIrpf(payslip.deductions ?? []);
    ...
    if (!contributionBasesAvailable) {
      findings.push({
        ...
        summary: `La nómina ${payslip.id} no expone bases de cotización en la fuente de datos.`,
    ...
    if (parseAmount(payslip.paymentPending) > 0) {
      findings.push({
        code: 'payslip_payment_pending',
        severity: 'info',
        category: 'payment',
        summary: `La nómina ${payslip.id} muestra importe pendiente de pago.`,
        evidence: { payslipId: payslip.id, paymentPending: payslip.paymentPending, paymentStatus: payslip.paymentStatus ?? null },
      });
    }

    return { ...payslip, contributionBasesAvailable, irpf };
  });
  ...
  if (snapshot.salaryRecordsReadError) {
    findings.push({
      ...
      summary: 'No se pudieron leer los salary-records manuales. Esto no invalida las nóminas calculadas.',
      evidence: { readError: snapshot.salaryRecordsReadError },
    });
    ...
    findings.push({
      ...
      severity: 'info',
      category: 'salary_record',
      summary: 'Existen salary-records manuales; se mantienen separados de las nóminas calculadas y no se usan como sustituto de ellas.',
      evidence: { count: snapshot.salaryRecords.length },
    });
  }
  ...
  return {
    ...
    period: {
      startDate: snapshot.period?.startDate ?? null,
      endDate: snapshot.period?.endDate ?? null,
    },
