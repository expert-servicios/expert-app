...
import {
  HoldedAuthError,
  HoldedPermissionError,
  HoldedRateLimitError,
} from '@/lib/integrations/holded/holded-errors';
...
describe('Holded labor API v2 client', () => {
  ...
  it('rejects an empty integration ID before auth resolution', async () => {
    await expect(createHoldedV2Client('   ')).rejects.toThrow(/integrationId is required/);
    expect(resolveHoldedAuth).not.toHaveBeenCalled();
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('serializes concurrent requests to avoid rate-limit bursts', async () => {
    ...
    const client = buildHoldedV2Client('key');
    await Promise.all([
      client.listEmployees({ search: 'one' }),
      client.listEmployees({ search: 'two' }),
      client.listEmployees({ search: 'three' }),
    ]);
  ...
  it('fails closed if a resource identifier is empty', async () => {
    const client = buildHoldedV2Client('key');
