...
Recepción: comprobar autenticidad sobre cuerpo original, fecha/ventana de replay y esquema; insertar en una bandeja persistente antes del acuse. `event_id` es el identificador canónico y estable del efecto de negocio, no un ID nuevo por intento de entrega: todos los reintentos del mismo efecto deben reutilizarlo, y cualquier adaptador de proveedor debe mapear su evento al mismo identificador canónico cuando represente la misma acción. Reenvío idéntico devuelve el resultado ya conocido; mismo ID con contenido distinto se rechaza. Cada proveedor usa su propia verificación oficial; no reutilizar la firma HMAC de Meta para OpenAI, Telegram o Work.
...
Tras verificación, `administrative_actions` conserva su máquina existente (`verifying → completed`); `internal_tasks` mantiene exclusivamente sus estados en español (`pendiente → en_progreso → completada`, o `cancelada` cuando proceda). El adaptador Work reclama y completa `internal_tasks`, pero no reutiliza los estados de `administrative_actions`. Calcular siguientes tareas por el blueprint versionado del servicio y dependencias satisfechas. Encolar solo pasos ya autorizados; si corresponde respuesta del cliente, autenticación o aprobación, fijar ese pendiente sin volver a pedir documentos ya recibidos. No encadenar ciegamente todas las tareas.
