...
export function ClientHoldedAdminPanel({ clientId }: { clientId: string }) {
  ...
  async function action(payload: Record<string, unknown>) {
    ...
    try {
      ...
      setApiKey(''); setShowKey(false); setConsent(false); setLaborConsent(false);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error de conexión');
    ...
    <div className="space-y-5">
      ...
      {integration?.status === 'active' ? (
        ...
        <section className="rounded-2xl border border-[#d8cbb5] bg-white p-6 shadow-sm">
          ...
          <label className="mt-4 flex items-start gap-2 text-xs leading-5 text-[#4b5563]"><input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} className="mt-1" /><span>Confirmo que el cliente ha autorizado la conexión de esta cuenta Holded a EXPERT para lectura de datos contables y que puedo registrar esta credencial en su expediente.</span></label>
          <label className="mt-3 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-xs leading-5 text-amber-900"><input type="checkbox" checked={laborConsent} onChange={(e) => setLaborConsent(e.target.checked)} className="mt-1" /><span><strong>Datos laborales:</strong> confirmo además que el cliente ha autorizado expresamente la lectura de empleados, contratos, nóminas y registros salariales. Si no se marca, estas capacidades permanecerán desactivadas aunque la API key las permita.</span></label>
          <div className="mt-5 flex justify-end"><button type="button" disabled={busy || apiKey.trim().length < 8 || !consent || !companyId} onClick={() => void action({ action: 'connect', companyId, apiKey: apiKey.trim(), consentConfirmed: true, laborReadAuthorized: laborConsent })} className="inline-flex items-center gap-2 rounded-xl bg-[#07111d] px-5 py-3 text-sm font-bold text-white disabled:opacity-40">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plug className="h-4 w-4" />}Verificar y conectar</button></div>
        </section>
      )}
    </div>
  );
