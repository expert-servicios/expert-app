...
import { serviceDisbursementPaymentConfirmed } from '@/lib/email/service-disbursement-confirmed';
import {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (quoteId) {
          ...
          if (!quote || quoteFetchError) {
            ...
            if (existingOrder) {
              ...
              const clientEmail = session.customer_email ?? (session.customer_details as { email?: string } | null)?.email;
      ...
      if (session.mode === 'payment' && (productType === 'service' || productType === 'cart') && session.payment_status === 'paid') {
        ...
        let catalogOrderMetadata: Record<string, unknown> = {
          ...
          payment_breakdown: {
            stripe_total_cents: paymentBreakdown.stripeTotalCents,
            professional_net_cents: paymentBreakdown.professionalNetCents,
            professional_vat_cents: paymentBreakdown.professionalVatCents,
            professional_gross_cents: paymentBreakdown.professionalGrossCents,
            disbursement_total_cents: paymentBreakdown.disbursementTotalCents,
            disbursement_keys: paymentBreakdown.disbursementKeys,
            disbursement_mandate_accepted: paymentBreakdown.mandateAccepted,
          },
        };
        ...
        if (!existingCatalogOrder) {
          ...
          if (newCatalogOrder?.metadata) {
            catalogOrderMetadata = newCatalogOrder.metadata as Record<string, unknown>;
          }
        } else {
        ...
        if (customerEmail) {
          ...
          } else if (paymentBreakdown.disbursementTotalCents > 0) {
            const tpl = serviceDisbursementPaymentConfirmed({
              name: customerName,
              service: serviceName,
              stripeTotalCents: paymentBreakdown.stripeTotalCents,
              professionalGrossCents: paymentBreakdown.professionalGrossCents,
              professionalNetCents: paymentBreakdown.professionalNetCents,
              professionalVatCents: paymentBreakdown.professionalVatCents,
              disbursementTotalCents: paymentBreakdown.disbursementTotalCents,
            });
            await sendEmail({
              to: customerEmail,
          ...
          const catalogJobId = await enqueueHoldedSync(supabaseAdmin, 'sync_order_holded', {
            ...
            amountEur: professionalAmountEur,
