...
async function assertClientCompany(admin: ReturnType<typeof getSupabaseAdmin>, clientId: string, companyId: string) {
  const { data, error } = await admin.from('profile_companies').select('company_id').eq('profile_id', clientId).eq('company_id', companyId).maybeSingle();
  if (error) throw error;
  return Boolean(data);
}

async function getIntegration(admin: ReturnType<typeof getSupabaseAdmin>, companyId: string) {
  const { data, error } = await admin.from('client_integrations').select(SAFE_COLUMNS).eq('provider', 'holded').eq('company_id', companyId).neq('status', 'revoked').order('created_at', { ascending: false }).limit(1).maybeSingle();
  if (error) throw error;
  return data;
}
...
async function detectAllPermissions(rawApiKey: string) {
  ...
  const [testResult, laborPermissions] = await Promise.all([client.testConnection(), detectHoldedLaborPermissions(rawApiKey)]);
  return {
    ...testResult,
    permissions: normalizeDetectedHoldedPermissions({ ...testResult.permissions, ...laborPermissions } as Partial<HoldedPermissions>),
  };
}
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!companyId || !z.string().uuid().safeParse(companyId).success) return NextResponse.json({ error: 'companyId inválido' }, { status: 400 });
    if (!(await assertClientCompany(auth.admin, clientId, companyId))) return NextResponse.json({ error: 'La empresa no está vinculada a este cliente' }, { status: 403 });
    return NextResponse.json({ integration: await getIntegration(auth.admin, companyId) });
  } catch (error) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!(await assertClientCompany(admin, clientId, companyId))) return NextResponse.json({ error: 'La empresa no está vinculada a este cliente' }, { status: 403 });
    ...
    if (parsed.data.action === 'connect') {
      ...
      let testResult;
      try { testResult = await detectAllPermissions(parsed.data.apiKey); }
      catch (error) { return NextResponse.json({ error: `No se pudo conectar con Holded: ${holdedErrorMessage(error)}` }, { status: 502 }); }
      ...
      const payload = {
        ...
        consent_version: 'admin-client-360-v2', disconnected_at: null, updated_at: now,
      };

      let integration;
      if (existing?.id) {
        const { data: previousSecret } = await admin.from('client_integration_secrets').select('encrypted_api_key').eq('integration_id', existing.id).maybeSingle();
        ...
        if (error || !data) {
          const rollback = previousSecret?.encrypted_api_key
            ? await admin.from('client_integration_secrets').upsert({ integration_id: existing.id, encrypted_api_key: previousSecret.encrypted_api_key, updated_at: now })
            : await admin.from('client_integration_secrets').delete().eq('integration_id', existing.id);
          if (rollback.error) console.error('[admin/client/holded] CRITICAL secret rollback error:', rollback.error.message);
      ...
      await admin.from('audit_logs').insert({ actor_id: actorId, action: 'holded.admin_connected', entity: 'companies', entity_id: companyId, metadata: { client_id: clientId, integration_id: integration.id, sync_mode: 'read_only', labor_read_authorized: parsed.data.laborReadAuthorized } }).then(() => {});
    ...
    const enabledPermissions = intersectHoldedReadPermissions(
      result.permissions,
      legacySafeRequestedPermissions(result.permissions, integration.permissions_enabled as Record<string, boolean> | null | undefined),
    );
    const { data: updated } = await admin.from('client_integrations').update({
      permissions_detected: result.permissions, permissions_enabled: enabledPermissions,
      last_success_at: result.ok ? now : integration.last_success_at,
      last_error: result.ok ? null : result.warnings.join('; '), updated_at: now,
