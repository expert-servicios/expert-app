...
export function generateStaticParams() {
  return RU_NATIONALITY_RESOURCES.filter((item) => item.kind === 'docs').map((item) => ({ slug: item.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const resource = getRuNationalityResource('docs', slug);
  if (!resource) return {};
  const ruPath = `/ru/docs/${resource.slug}`;
  return {
...
export default async function RuDocsPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const resource = getRuNationalityResource('docs', slug);
