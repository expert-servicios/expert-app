...
const SERVICE_SLUG = 'holded-modulo-laboral';
const ES_URL = 'https://expertconsulting.es/servicios/holded/holded-modulo-laboral';
const RU_PATH = '/ru/uslugi/kadrovyy-modul-holded';
...
const service = (() => {
  ...
  if (!canonical?.stripePriceId || !canonical.price) {
    throw new Error('Canonical holded-modulo-laboral service is not configured for checkout.');
  }
  return {
    ...canonical,
...
export const metadata: Metadata = {
  title: 'Кадровый модуль Holded — настройка | EXPERT',
  description:
    `Внедрение кадрового модуля Holded: зарплаты, договоры и учёт сотрудников с интеграцией в бухгалтерию. ${service.price}.`,
  alternates: {
    canonical: RU_URL,
    languages: {
      'es-ES': ES_URL,
  ...
  openGraph: {
    title: 'Кадровый модуль Holded | EXPERT',
    description: 'Зарплаты, договоры и приём/увольнение сотрудников в одной системе с бухгалтерией.',
    url: RU_URL,
    type: 'website',
    locale: 'ru_RU',
    siteName: 'EXPERT',
...
const includes = [
  'Начальная настройка кадрового модуля',
  'Загрузка сотрудников и данных для расчёта зарплаты',
  'Интеграция с бухгалтерией и казначейством (tesorería)',
  'Тестовый расчёт зарплат',
  'Поддержка по email в течение 15 дней',
];

const faqs = [
  {
    q: 'Входит ли ежемесячное ведение зарплат?',
    a: 'Нет. Эта услуга покрывает настройку и внедрение модуля. Ежемесячное ведение зарплат — отдельная услуга.',
  },
  {
    q: 'Нужно ли уже иметь настроенный остальной Holded?',
    a: 'Да, рекомендуем предварительно настроить как минимум компанию и бухгалтерию перед активацией кадрового модуля.',
  },
];

export default function RuHoldedModuloLaboralPage() {
  const serviceJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: 'Кадровый модуль Holded',
    description:
      'Внедрение кадрового модуля Holded для расчёта зарплат, трудовых договоров, приёма и увольнения сотрудников с прямой интеграцией с бухгалтерией. Включает начальную настройку, загрузку сотрудников и тестовый прогон.',
    serviceType: 'Holded',
    provider: {
      '@type': 'Organization',
      name: 'EXPERT',
      ...
      <section className="bg-[#0D1B2A] px-6 py-12 text-[#F8F6F1] md:py-16">
        <div className="mx-auto max-w-5xl">
          ...
          <h1 className="mt-4 font-serif text-3xl font-bold leading-tight md:text-4xl lg:text-5xl">
            Кадровый модуль Holded
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/65">
            Внедряем кадровый модуль Holded, чтобы вы могли вести расчёт зарплат, трудовые договоры,
            приём и увольнение сотрудников с прямой интеграцией с бухгалтерией — включая подачу
            сведений в Seguridad Social. Включает начальную настройку, загрузку сотрудников и тестовый
            прогон.
          </p>
