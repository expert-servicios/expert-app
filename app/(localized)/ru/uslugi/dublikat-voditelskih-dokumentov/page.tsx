...
const SERVICE_SLUG = 'duplicado-permiso';
const ES_URL = 'https://expertconsulting.es/servicios/trafico-capitania-maritima/duplicado-permiso';
const RU_PATH = '/ru/uslugi/dublikat-voditelskih-dokumentov';
...
const service = (() => {
  ...
  if (!canonical) {
    throw new Error('Canonical duplicado-permiso service is missing from the catalog.');
  }
  return canonical;
})();

const DURATION_RU = '2–5 рабочих дней';

const budgetHref = `/solicitar-presupuesto?servicio=${SERVICE_SLUG}`;
const complexBudgetHref = `${budgetHref}&tipo=caso-complejo`;

export const metadata: Metadata = {
  title: 'Дубликат документов ГИБДД Испании (DGT) | EXPERT',
  description:
    `Оформление дубликата водительского удостоверения, permiso de circulación или ficha técnica в испанском DGT. ${service.price}.`,
  alternates: {
    canonical: RU_URL,
    languages: {
      'es-ES': ES_URL,
  ...
  openGraph: {
    title: 'Дубликат документов DGT | EXPERT',
    description: 'Оформление дубликата водительского удостоверения, permiso de circulación или ficha técnica.',
    url: RU_URL,
    type: 'website',
    locale: 'ru_RU',
    siteName: 'EXPERT',
...
const includes = [
  'Подача заявления в DGT',
  'Получение соответствующего дубликата документа',
];

const faqs = [
  {
    q: 'Можно ли водить машину, пока не пришёл дубликат?',
    a: 'Если DGT выдаёт временное разрешение (autorización provisional), оно позволяет управлять автомобилем в Испании в течение срока его действия, пока не придёт постоянный документ. Мы уточняем конкретную справку по вашему делу.',
  },
];

export default function RuDuplicadoPermisoPage() {
  const serviceJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: 'Дубликат документов ГИБДД Испании (DGT)',
    description:
      'Оформление дубликата водительского удостоверения, permiso de circulación или ficha técnica/eITV в испанском DGT.',
      ...
      <section className="bg-[#0D1B2A] px-6 py-12 text-[#F8F6F1] md:py-16">
        <div className="mx-auto max-w-5xl">
          ...
          <h1 className="mt-4 font-serif text-3xl font-bold leading-tight md:text-4xl lg:text-5xl">
            Дубликат документов ГИБДД Испании (DGT)
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/65">
            Оформляем дубликаты и переоформление документов, связанных с дорожным движением, в зависимости от
            документа и причины: водительское удостоверение, permiso de circulación и ficha técnica/eITV.
            Проверяем нужный канал подачи, размер пошлины и действительно ли требуется дубликат, а не другая
            процедура.
          </p>
