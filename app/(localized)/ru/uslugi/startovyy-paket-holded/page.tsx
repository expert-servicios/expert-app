...
const SERVICE_SLUG = 'holded-pack-starter';
const ES_URL = 'https://expertconsulting.es/servicios/holded/holded-pack-starter';
const RU_PATH = '/ru/uslugi/startovyy-paket-holded';
...
const service = (() => {
  ...
  if (!canonical?.stripePriceId || !canonical.price) {
    throw new Error('Canonical holded-pack-starter service is not configured for checkout.');
  }
  return {
    ...canonical,
...
export const metadata: Metadata = {
  title: 'Стартовый пакет Holded — настройка под ключ | EXPERT',
  description:
    `Настройка Holded под ключ как Holded Solution Partner: компания, выставление счетов, банки и Open Banking. ${service.price}.`,
  alternates: {
    canonical: RU_URL,
    languages: {
      'es-ES': ES_URL,
  ...
  openGraph: {
    title: 'Стартовый пакет Holded | EXPERT',
    description: 'Настройка компании, выставления счетов, банков и Open Banking за 1–2 недели.',
    url: RU_URL,
    type: 'website',
    locale: 'ru_RU',
    siteName: 'EXPERT',
...
const includes = [
  'Начальная настройка аккаунта',
  'Настройка компании, выставления счетов и банков',
  'Банковское подключение (Open Banking)',
  'Поддержка по email в течение 30 дней',
];

const faqs = [
  {
    q: 'Нужно ли уже иметь оформленную подписку на Holded?',
    a: 'Нет. Как Holded Solution Partner мы можем оформить доступ и активировать бесплатный 14-дневный пробный период перед началом внедрения.',
  },
  {
    q: 'Сколько занимает Стартовый пакет?',
    a: 'От 1 до 2 недель с момента получения документации о вашей компании.',
  },
];

export default function RuHoldedPackStarterPage() {
  const serviceJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: 'Стартовый пакет Holded',
    description:
      'Настройка аккаунта Holded под ключ: компания, выставление счетов, банки и подключение Open Banking. Поддержка по email в течение 30 дней.',
    serviceType: 'Holded',
    provider: {
      '@type': 'Organization',
      name: 'EXPERT',
      ...
      <section className="bg-[#0D1B2A] px-6 py-12 text-[#F8F6F1] md:py-16">
        <div className="mx-auto max-w-5xl">
          ...
          <h1 className="mt-4 font-serif text-3xl font-bold leading-tight md:text-4xl lg:text-5xl">
            Стартовый пакет Holded
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/65">
            Настраиваем ваш аккаунт Holded для полноценной работы: компания, выставление счетов,
            банковские счета и подключение Open Banking. Подходит для autónomos и малого бизнеса,
            которые только начинают работать с Holded.
          </p>
