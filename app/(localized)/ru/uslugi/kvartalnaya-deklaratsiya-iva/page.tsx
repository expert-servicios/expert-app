...
const SERVICE_SLUG = 'iva-trimestral';
const ES_URL = 'https://expertconsulting.es/servicios/declaraciones-impuestos/iva-trimestral';
const RU_PATH = '/ru/uslugi/kvartalnaya-deklaratsiya-iva';
...
if (!getCatalogService(SERVICE_SLUG)) {
  throw new Error('Canonical iva-trimestral service is missing from the catalog.');
}

const PRICE_RU = 'От 60 € + IVA / за квартал';
const DURATION_RU = '2–3 рабочих дня';

const budgetHref = `/solicitar-presupuesto?servicio=${SERVICE_SLUG}`;
const complexBudgetHref = `${budgetHref}&tipo=caso-complejo`;

export const metadata: Metadata = {
  title: 'Квартальная декларация IVA в Испании | EXPERT',
  description:
    `Подготовка и подача квартальной декларации IVA (Modelo 303) и годового отчёта Modelo 390. ${PRICE_RU}.`,
  alternates: {
    canonical: RU_URL,
    languages: {
      'es-ES': ES_URL,
  ...
  openGraph: {
    title: 'Квартальная декларация IVA | EXPERT',
    description: 'Modelo 303 каждый квартал и годовой отчёт Modelo 390.',
    url: RU_URL,
    type: 'website',
    locale: 'ru_RU',
    siteName: 'EXPERT',
...
const includes = [
  'Проверка выставленных и полученных счетов',
  'Подготовка Modelo 303',
  'Подача в установленный срок',
  'Годовой итоговый отчёт Modelo 390',
  'Напоминания о сроках подачи',
];

const faqs = [
  {
    q: 'Какие сроки подачи по кварталам?',
    a: 'Общий порядок — ежеквартальный, но перед подачей мы всегда проверяем точную дату окончания периода по действующему календарю AEAT.',
  },
  {
    q: 'Можно ли я сам буду вести учёт счетов, а вы только подадите декларацию?',
    a: 'Да, вы можете присылать нам реестр счетов, а мы возьмём на себя расчёт налога и подачу декларации.',
  },
];

export default function RuIvaTrimestralPage() {
  const serviceJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: 'Квартальная декларация IVA',
    description:
      'Подготовка и подача квартальной декларации IVA (Modelo 303), годового отчёта Modelo 390 и связанных форм.',
      ...
      <section className="bg-[#0D1B2A] px-6 py-12 text-[#F8F6F1] md:py-16">
        <div className="mx-auto max-w-5xl">
          ...
          <h1 className="mt-4 font-serif text-3xl font-bold leading-tight md:text-4xl lg:text-5xl">
            Квартальная декларация IVA
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/65">
            Готовим и подаём вашу квартальную декларацию по IVA (Modelo 303), годовой итоговый отчёт
            (Modelo 390) и другие связанные формы. Услуга включает проверку выставленных и полученных
            счетов для корректного расчёта налога.
          </p>
