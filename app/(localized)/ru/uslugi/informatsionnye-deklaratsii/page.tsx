...
const SERVICE_SLUG = 'modelos-informativos';
const ES_URL = 'https://expertconsulting.es/servicios/declaraciones-impuestos/modelos-informativos';
const RU_PATH = '/ru/uslugi/informatsionnye-deklaratsii';
...
if (!getCatalogService(SERVICE_SLUG)) {
  throw new Error('Canonical modelos-informativos service is missing from the catalog.');
}

const PRICE_RU = 'От 50 € + IVA / за форму';
const DURATION_RU = '2–4 рабочих дня';

const budgetHref = `/solicitar-presupuesto?servicio=${SERVICE_SLUG}`;
const complexBudgetHref = `${budgetHref}&tipo=caso-complejo`;

export const metadata: Metadata = {
  title: 'Информационные декларации в Испании | EXPERT',
  description:
    `Подготовка и подача Modelo 347, 349, 180, 190 и других годовых информационных деклараций в AEAT. ${PRICE_RU}.`,
  alternates: {
    canonical: RU_URL,
    languages: {
      'es-ES': ES_URL,
  ...
  openGraph: {
    title: 'Информационные декларации | EXPERT',
    description: 'Modelo 347, 349, 180, 190 и другие годовые информационные формы.',
    url: RU_URL,
    type: 'website',
    locale: 'ru_RU',
    siteName: 'EXPERT',
...
const includes = [
  'Проверка и сверка данных с бухгалтерией',
  'Подготовка соответствующей формы',
  'Подача в срок в AEAT',
  'Копия подтверждения подачи',
];

const faqs = [
  {
    q: 'Что будет, если подать информационную декларацию с опозданием?',
    a: 'За несвоевременную подачу предусмотрены санкции. Мы заранее предупреждаем о сроках, чтобы избежать штрафов и пени.',
  },
];

export default function RuModelosInformativosPage() {
  const serviceJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: 'Информационные декларации',
    description:
      'Подготовка и подача основных годовых информационных деклараций: Modelo 347, 349, 180, 190 и другие.',
      ...
      <section className="bg-[#0D1B2A] px-6 py-12 text-[#F8F6F1] md:py-16">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/65">
            Готовим и подаём основные годовые информационные декларации: операции с третьими лицами
            (Modelo 347), внутриевропейские операции (Modelo 349), удержания по аренде (Modelo 180),
            удержания по заработной плате (Modelo 190) и другие.
          </p>
