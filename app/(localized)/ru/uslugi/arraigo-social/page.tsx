...
const documents = [
  ...
  },
  {
    ...
    items: [
      ...
      'Если подачу выполняет представитель: документ, подтверждающий допустимость и объём представительства для конкретной процедуры.',
    ],
  },
];
...
const processSteps = [
  ...
  {
    title: 'Проверка права на подачу и представительства',
    text: 'Сначала определяем, кто вправе подать заявление по конкретному основанию. EXPERT подаёт expediente только если такой способ подачи и представительство юридически допустимы и надлежащим образом подтверждены. В остальных случаях подготавливаем expediente и сопровождаем личную/электронную подачу заявителя.',
  },
  {
...
const faqs = [
  ...
  {
    q: 'Может ли EXPERT подать Arraigo Social вместо меня?',
    a: 'Не автоматически. Официальная памятка указывает заявителя лично либо законного представителя для несовершеннолетнего/недееспособного. Перед подачей мы проверяем конкретный способ подачи и, если представительство допустимо, документ, которым оно должно быть подтверждено.',
  },
];
...
export default function RuArraigoSocialPage() {
  const serviceJsonLd = {
    ...
    description:
      'Сопровождение Arraigo Social: проверка 2 лет пребывания, основания, документации, EX-10 и допустимого способа подачи в Extranjería.',
