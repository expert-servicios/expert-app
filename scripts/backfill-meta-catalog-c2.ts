...
import { SUBSCRIPTION_ONLY_SLUGS, CATEGORY_OVERRIDES, MONTHLY_PLANS, isArchivedService } from '../lib/marketing/meta-catalog-archive';
...
// dead link) when a card hasn't been generated for this slug yet.
function cardImageUrl(slug: string): string | null {
  return existsSync(path.join(CARDS_DIR, `${slug}.png`)) ? `/catalog/servicios/${slug}.png` : null;
}
...
async function main() {
  ...
  for (const service of services) {
    ...
    // Archived services intentionally have no card (see generate-service-cards.ts),
    // so only a missing image on an active service is worth flagging.
    const imageUrl = cardImageUrl(service.slug);
  ...
  for (const plan of MONTHLY_PLANS) {
    if (APPLY && admin) {
      await upsertCatalogEntry(admin, {
        ...
        imageUrl: cardImageUrl(plan.slug),
